...
func (s *rpcServer) reSubscribe(config Options) error {
	for sb := range s.subscribers {
		...
		var err error
		if sb.Options().NewBroker && config.NewBroker != nil {
			sub, err = config.NewBroker.Subscribe(sb.Topic(), s.HandleEvent(sb.Topic()), opts...)
			if err != nil {
				config.Logger.Logf(log.ErrorLevel, "Unable to subscribing to topic: %s", sb.Topic())
				continue
			}
		} else {
